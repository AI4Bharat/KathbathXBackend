 ...
 * @param match Match object representing the single record
 */
export async function getSingleWithAandW<TableName extends DbTableName>(
  tableName: TableName,
  access_code: DbObjectType<TableName>,
  id:DbObjectType<TableName>

): Promise<DbRecordType<TableName>> {
  const p = 1;
  const response = await knex(tableName).where(id).orWhere(access_code).first();
