  ...
  tables: {
    ...
    microtask: {
      columns: [
        ...
        ['deadline', ['timestamp'], 'not unique', 'nullable', 'mutable'],
        ['base_credits', ['float'], 'not unique', 'not nullable', 'mutable'],
    ...
    microtask_assignment: {
      columns: [
        ...
        ['max_base_credits', ['float'], 'not unique', 'not nullable', 'mutable'],
        ['base_credits', ['float'], 'not unique', 'not nullable', 'mutable'],
...
boxTables.forEach((table) => {
  ...
  karyaServerDb.tables[table].triggers = [computeIDTrigger(table)];
});
