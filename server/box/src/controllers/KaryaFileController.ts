...
import { BasicModel, mainLogger, uploadBlobFromFileWithName } from '@karya/common';
import fs from 'fs';
...
export const getFile: KaryaFileGetMiddleware = async (ctx) => {
  ...
  const filePath = `${sharedPath}/${local_folder}/${cname}/${fileName}`;
...
export const submitFile: KaryaFileSubmitMiddleware = async (ctx, next) => {
  try {
    ...
    const destination = `${sharedPath}/${folder}/${container_name}/${name}`;
    await fs.promises.copyFile(ctx.state.filePath, destination);

    // Include box_id
    ctx.state.karya_file.box_id = ctx.state.entity.box_id;
    
    // Upload the file to the cloud storage; Tahir
    const blobURL = await uploadBlobFromFileWithName(ctx.state.karya_file.container_name!, ctx.state.karya_file.name!, ctx.state.filePath);    
    
    // Delete the temporary file
    await fs.promises.unlink(ctx.state.filePath);
    ctx.state.karya_file.in_server = true
    ctx.state.karya_file.url = blobURL
    ctx.state.karya_file.in_server = true
