...
async function verify(client_id:string, jwtToken:string) {
...
class LoginRegister extends React.Component<SignUpProps, SignUpState> {
   ...
   */
  onGoogleSignUpSuccess = (googleUserPayload: any) => {
    console.log(googleUserPayload)
    const googleUser = verify(googleUserPayload.client_id,googleUserPayload.credential)
    ...
    googleUser.then((res)=>{
      console.log(res)
      // const profile = res.name;
  
      wp.access_code = this.state.wp.access_code;
      wp.full_name =  res.name;
      wp.email = res.email;
  
    })
    const showProfileDetails = true;
    const id_token = googleUserPayload.credential;
