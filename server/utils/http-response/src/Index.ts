...
import { ValidationOutput } from '@karya/core';
 ...
 */
export function UnprocessableContent(ctx: ExtendableContext, messages: ValidationOutput | ValidationOutput[]) {
  // GenericError(ctx, messages, 422, 'Unprocessable Content');
  const errorBody = {
 ...
 */
export function Conflict(ctx: ExtendableContext, message: string) {
  GenericError(ctx, message, 409, 'Conflict');
}
