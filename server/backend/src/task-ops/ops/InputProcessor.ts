  ...
  await BBPromise.mapSeries(groups, async (group) => {
    ...
    await BBPromise.mapSeries(microtasks, async (microtask) => {
      ...
      if (mtRecord.input.files) {
        ...
        // Since the server and box are now merged, directly copying the created microtask.tgz into required directory, saving unnecessary download
        const microtaskInputFilePath = `${path.dirname(taskFolder).replace('task-input','microtask-input')}/${inputTgzFileName}`;

        await fsp.copyFile(inputTgzFilePath,microtaskInputFilePath);
