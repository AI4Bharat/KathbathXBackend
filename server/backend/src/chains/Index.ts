...
import { handleNewlyCompletedAssignmentCustom } from '../task-ops/policies/Index';
...
import { speechValidationMultiChain } from './chains/SpeechValidationMulti';
import { speechSuperValidationChain } from './chains/SpeechSuperValidation';
...
export const backendChainMap: { [key in ChainName]: BackendChainInterface<any, any> } = {
  ...
  SPEECH_VALIDATION_MULTI: speechValidationMultiChain,
  SPEECH_SUPERVALIDATION: speechSuperValidationChain,
...
export async function executeBackwardLink(microtasks: ChainedMicrotaskRecordType[], link: TaskLinkRecord) {
  ...
  if (link.blocking) await handleNewlyCompletedAssignmentCustom(verifiedAssignments[0], fromTask);
}
