...
async function processInputFile(
  ...
  const verifications: { sentence:string, recording: string, duration: number, chain: any }[] = jsonData!;
  
  // Extract the microtasks
  const microtasks = await BBPromise.mapSeries(verifications, async ({ sentence, recording, duration, chain }) => {
...
export const backendSpeechDVScenario: IBackendScenarioInterface<BaseSpeechDVScenario> = {
  ...baseSpeechDVScenario,
  processInputFile,
